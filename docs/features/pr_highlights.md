...
## 1. Transaction isolation & atomicity (one-tx-per-cycle)

- ⭐ [`erigon#6348`](https://github.com/erigontech/erigon/pull/6348) · 2022-12 — **Run a whole non-initial sync cycle in a single `RwTx`** (broke the dependency on `CurrentHeader`). Makes a multi-stage cycle one atomic read-write transaction.
  *Angle: "One transaction per cycle: making a multi-stage pipeline atomic."*
- ⭐ [`erigon#7713`](https://github.com/erigontech/erigon/pull/7713) · 2023 — **Fix `canRunCycleInOneTransaction`: readers could see partial writes.** A read-isolation guarantee — no observing a half-applied cycle.
  *Angle: "Read isolation in a staged pipeline: never expose partial writes."*
- ⭐ [`erigon#1897`](https://github.com/erigontech/erigon/pull/1897) · 2021 — **Move staged sync to a transaction** (+862/−915): the foundational refactor putting the whole loop under explicit DB transactions.
- Transaction-scoping ETL/stages: [`#1660`](https://github.com/erigontech/erigon/pull/1660) etl→rwtx · [`#1727`](https://github.com/erigontech/erigon/pull/1727)/[`#1726`](https://github.com/erigontech/erigon/pull/1726) history/IH→rwtx · [`#1692`](https://github.com/erigontech/erigon/pull/1692) drop etl `onLoadCommit` hook · [`#1806`](https://github.com/erigontech/erigon/pull/1806) exec-unwind ETL ("solves the `TXN_FULL` problem").

## 2. Storage engine internals (MDBX / LMDB)
...
## 3. Memory: off-heap, mmap & madvise
...
## 6. Compression & encoding algorithms
...
- ⭐ [`erigon#14604`](https://github.com/erigontech/erigon/pull/14604) · 2025 — **Snappy for `commitment.v`**: 2× smaller, avoids "over-flooding the shared dictionary with good patterns."
...
## 7. Indexing & minimal-perfect-hashing (RecSplit / Elias-Fano)
...
## 8. Bitmaps & inverted indices (logs, receipts, traces)
...
## 9. On-disk / snapshot (`.seg`) formats & write amplification
...
## 10. Snapshot distribution: Downloader, BitTorrent & webseed
...
## 11. Pruning, history retention & file merge
...
## 12. Trie & commitment — state root / Hex Patricia
...
## 13. Erigon3 state model — txNum & domains
...
- ⭐ [`erigon#8855`](https://github.com/erigontech/erigon/pull/8855) / [`#8853`](https://github.com/erigontech/erigon/pull/8853) · 2023 — **`UnwindTo` only moves to blocks with a committed state root** — a consistency guarantee on where you may rewind.

## 14. KV streaming & remote DB (gRPC)
...
## 15. Execution / EVM micro-architecture
...
- [`#20372`](https://github.com/erigontech/erigon/pull/20372) index-based range over log topics (avoid copying 32-byte `common.Hash` each iteration) · see also `#21858` (typed-tx `Hash()` allocs, §3).
...
7. **`#6348` + `#7713`** — one transaction per cycle and never showing partial writes.
